# -*- coding: utf-8 -*-
"""
real_pipeline/training/train_flood_models_v2.py
================================================
Flood research baseline training v2 -- uses flood_training_v2.parquet
...
KEY DIFFERENCES FROM v1:
  - Uses flood_training_v2.parquet (10 events, 8 regions) vs v1 (3 events, 3 regions)
...
Outputs:
  models_real/flood_v2_research_model.pkl
  models_real/flood_v2_research_metadata.json
  data_real/evaluation_v2/fold_metrics.csv
  data_real/evaluation_v2/event_metrics.csv
  data_real/evaluation_v2/threshold_analysis.csv
  data_real/evaluation_v2/feature_importance.csv
  data_real/evaluation_v2/model_card_flood_v2.md
"""
...
EVAL_DIR   = PROJECT_ROOT / "data_real" / "evaluation_v2"
MODELS_DIR = PROJECT_ROOT / "models_real"

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("train_flood_v2")
...
def load_data() -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Flood v2 matrix missing: {path}")
...
def save_best_model(best: dict, feature_list: list, df: pd.DataFrame,
    ...
    pkl_path = MODELS_DIR / "flood_v2_research_model.pkl"
    ...
    meta_path = MODELS_DIR / "flood_v2_research_metadata.json"
...
def write_model_card(feature_list: list, rain_ok: bool, loeo_results: dict,
    ...
    lines = [
        "# ResQ Shield -- Flood Research Model v2  [EXPERIMENTAL_BASELINE_ONLY]",
        "",
    ...
    card_path = EVAL_DIR / "model_card_flood_v2.md"
...
def main():
    log.info("="*65)
    log.info("ResQ Shield -- Flood Research Models v2  [EXPERIMENTAL_BASELINE_ONLY]")
    log.info("Matrix: flood_training_v2.parquet  (10 events, 8 regions)")
    log.info("="*65)
