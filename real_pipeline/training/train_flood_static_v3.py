# -*- coding: utf-8 -*-
"""
real_pipeline/training/train_flood_static.py
================================================
Flood research static susceptibility baseline.
(10 positive events, 8 regions).

STATUS:  EXPERIMENTAL_BACKGROUND_PRIOR
         Represents WHERE susceptibility, NOT current hazard state.

MODEL_SAVE_PATH = PROJECT_ROOT / "models_real" / "flood_static_v3_research_model.pkl"
META_SAVE_PATH  = PROJECT_ROOT / "models_real" / "flood_static_v3_research_metadata.json"
  data_real/evaluation_v3/static_fold_metrics.csv
  data_real/evaluation_v3/static_event_metrics.csv
"""
...
MATRIX_FILE = PROJECT_ROOT / "data_real" / "features" / "flood_training_v4.parquet"
EVAL_DIR   = PROJECT_ROOT / "data_real" / "evaluation_v3"
MODELS_DIR = PROJECT_ROOT / "models_real"

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("train_flood_static")
...
TERRAIN_FEATURES = ["elev_mean_m", "slope_mean_deg"]
HYDRO_FEATURES = ["nearest_river_m", "total_upstream_area_skm"]
INFRA_FEATURES = ["nearest_hospital_m"]
...
BLACKLIST = {
    ...
    "data_type", "rain_event_sum_mm", "rain_event_max_mm", "rain_event_mean_mm", "rain_event_p90_mm", "n_days_valid_chirps", "chirps_coverage_pct", "ant_3d_mm", "n_days_ant_3d", "ant_7d_mm", "n_days_ant_7d", "ant_14d_mm", "n_days_ant_14d",
}
...
def load_data() -> pd.DataFrame:
    path = MATRIX_FILE
    if not path.exists():
        raise FileNotFoundError(f"Flood v4 matrix missing: {path}")
...
def select_features(df: pd.DataFrame, terrain_ok: bool) -> list:
    feats = []
    ...
    log.info("  Contextual features: INCLUDED")

    # Strict dynamic leakage assertion
    dynamic_substrings = ["rain", "ant", "event", "coverage"]
    for f in feats:
        for sub in dynamic_substrings:
            if sub in f.lower():
                raise ValueError(f"CRITICAL ERROR: Dynamic/leaked feature '{f}' found in static feature list!")
...
def save_best_model(best: dict, feature_list: list, df: pd.DataFrame,
                    loeo_results: dict, loro_results: list):
    MODELS_DIR.mkdir(parents=True, exist_ok=True)
    pkl_path = MODELS_DIR / "flood_static_v3_research_model.pkl"
    ...
    metadata = {
        "schema_version": "v3",
        "model_version": "flood_static_v3_research",
        ...
        "rain_features_included": False,
        "rain_exclusion_reason": "Static prior strictly excludes event-time predictors.",
        ...
        "limitations": [
            ...
            "Rain features excluded -- Static prior strictly excludes event-time predictors.",
    ...
    meta_path = MODELS_DIR / "flood_static_v3_research_metadata.json"
    ...
    write_model_card(feature_list, loeo_results, loro_results, best, df)


def write_model_card(feature_list: list, loeo_results: dict,
    ...
    lines = [
        ...
        f"- **Rain features included**: STRICTLY EXCLUDED (STATIC PRIOR)",
    ...
    lines += [
        ...
        f"- `RAIN_FEATURES: STRICTLY EXCLUDED`",
...
def main():
    log.info("="*65)
    log.info("ResQ Shield -- Flood Research Static Model  [EXPERIMENTAL_BACKGROUND_PRIOR]")
    log.info("Matrix: flood_training_v3.parquet")
    log.info("="*65)
    ...
    available = select_features(df, terrain_ok)
    
    # Drop rows missing features
    mask = df[available].notna().all(axis=1)
    df = df[mask].copy()
    log.info(f"Dropped rows with missing static features. Remaining rows: {len(df):,}")

    if len(df) == 0:
        log.error("No data left after dropping NaNs! Aborting.")
        return

    models = build_models(RANDOM_SEED)
    loeo_results = run_loeo(df, available, models)

    best = loeo_results["best_model"]
    log.info(f"\nBest model: {best['name']}  LOEO macro PR-AUC={best['score']:.4f}")

    models_loro = build_models(RANDOM_SEED)
    loro_results = run_loro(df, available, best["name"], models_loro)

    loeo_macro = macro_summary(loeo_results["metrics"], "LOEO", best["name"])

    X_full = prepare_features(df, available, "drop")
    y_full = df["label"].values
    final_model = build_models(RANDOM_SEED)[best["name"]]
    final_model.fit(X_full, y_full)
    
    best["model"] = final_model
    save_best_model(best, available, df, loeo_results, loro_results)

    import pandas as pd
    df_folds = pd.DataFrame(loeo_results["metrics"])
    df_folds.to_csv(EVAL_DIR / "static_v2_fold_metrics.csv", index=False)
    
    df_events = pd.DataFrame(loeo_results["event_metrics"])
    df_events.to_csv(EVAL_DIR / "static_v2_event_metrics.csv", index=False)
