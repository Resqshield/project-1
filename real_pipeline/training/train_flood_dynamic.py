# -*- coding: utf-8 -*-
"""
real_pipeline/training/train_flood_dynamic.py
================================================
Flood research dynamic hazard baseline.
(10 positive events, 8 regions).

STATUS:  EXPERIMENTAL_DYNAMIC_HAZARD
         Represents WHEN current hazard state.

Outputs:
  models_real/flood_dynamic_research_model.pkl
  models_real/flood_dynamic_research_metadata.json
  data_real/evaluation_v3/dynamic_dynamic_fold_metrics.csv
  data_real/evaluation_v3/dynamic_dynamic_event_metrics.csv
"""
...
log = logging.getLogger("train_flood_dynamic")
...
def select_features(df: pd.DataFrame, rain_ok: bool) -> list:
    feats = []
    if rain_ok:
        for f in RAIN_FEATURES:
            if f in df.columns:
                feats.append(f)
        log.info("  Rain features (Dynamic): INCLUDED")
    else:
        log.error("  Rain features: EXCLUDED (parity violation)")
        raise ValueError("Cannot train a dynamic hazard model without dynamic rainfall features!")

    # Strict static leakage assertion
    static_substrings = ["elev", "slope", "lat", "lon", "distance", "accumulation"]
    for f in feats:
        for sub in static_substrings:
            if sub in f.lower():
                raise ValueError(f"CRITICAL ERROR: Static/leaked feature '{f}' found in dynamic feature list!")

    blocked = [f for f in feats if f in BLACKLIST]
    assert not blocked, f"BLACKLISTED features in predictor list: {blocked}"
    log.info(f"\nFinal dynamic feature list ({len(feats)} features):")
    for f in feats:
...
class RainfallThresholdBaseline(BaseEstimator, ClassifierMixin):
    def __init__(self, threshold_mm=100.0):
        self.threshold_mm = threshold_mm
    def fit(self, X, y):
        return self
    def predict_proba(self, X):
        import numpy as np
        if "rain_event_sum_mm" in X.columns:
            probs = (X["rain_event_sum_mm"] > self.threshold_mm).astype(float).values
        else:
...
def build_models(random_state: int = 42) -> dict:
    return {
        "RainfallThreshold": RainfallThresholdBaseline(threshold_mm=100.0),
...
def save_best_model(best: dict, feature_list: list, df: pd.DataFrame,
    ...
    pkl_path = MODELS_DIR / "flood_dynamic_research_model.pkl"
    ...
    metadata = {
        ...
        "created_at": datetime.now(timezone.utc).isoformat(),
        "readiness_label": "EXPERIMENTAL_DYNAMIC_HAZARD",
        "limitations": [
    ...
    meta_path = MODELS_DIR / "flood_dynamic_research_metadata.json"
...
def write_model_card(feature_list: list, rain_ok: bool, loeo_results: dict,
    ...
    lines = [
        "# ResQ Shield -- Flood Research Model v3  [EXPERIMENTAL_DYNAMIC_HAZARD]",
        "",
    ...
    card_path = EVAL_DIR / "model_card_flood_dynamic.md"
...
def main():
    log.info("="*65)
    log.info("ResQ Shield -- Flood Research Dynamic Model  [EXPERIMENTAL_DYNAMIC_HAZARD]")
    ...
    feature_list = select_features(df, rain_ok)
    if not feature_list:
    ...
    all_metrics_df.to_csv(EVAL_DIR / "dynamic_fold_metrics.csv", index=False)
    em_df.to_csv(EVAL_DIR / "dynamic_event_metrics.csv", index=False)
    ...
    log.info(f"  GATE_H: {'PASS' if gate_pass else 'PARTIAL'} (EXPERIMENTAL_DYNAMIC_HAZARD)")
    log.info("="*65)
    log.info("FINAL STATUS: EXPERIMENTAL_DYNAMIC_HAZARD -- NOT_OPERATIONAL")
